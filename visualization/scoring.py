...
def eval_swap(current_path, current_score, i, j, Pictures):
    # Evalue le gain en effectuant le swap i,j sur le chemin L
    a, b = current_path[i], current_path[j]
    a, b = pic_to_tags(a, Pictures), pic_to_tags(b, Pictures)
    c, d = [] if i == 0 else pic_to_tags(current_path[i - 1], Pictures), [] if j == len(
        current_path) - 1 else pic_to_tags(current_path[j + 1], Pictures)
    return current_score + score(c, b) + score(a, d) - score(c, a) - score(b, d)